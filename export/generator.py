"""PDF and DOCX comparison report generation for entity comparisons.

Deep module: one interface serves HTTP, CLI, and batch callers.
The font path is resolved from env or OS defaults.
"""
...
def _resolve_font_path() -> str:
    """Resolve font path from env or OS defaults."""
    font_path = _os.environ.get("KG_FONT_PATH", "")
    if font_path:
        return font_path
    windir = _os.environ.get("WINDIR", "")
    if windir:
        return windir + r"\Fonts\msyh.ttc"
    # Linux fallbacks
    for candidate in [
...
def make_filename(entity_a: Dict[str, Any], entity_b: Dict[str, Any], ext: str) -> str:
...
def generate_comparison_pdf(
    ...
    if not _FONT_PATH:
        raise RuntimeError(
            "No CJK font found. Set KG_FONT_PATH env var to a Chinese-capable font file "
            "(e.g. C:\\Windows\\Fonts\\msyh.ttc on Windows, "
            "/usr/share/fonts/noto-cjk/NotoSansCJK-Regular.ttc on Linux)."
        )
